import Image from "next/image";
import Container from "../SectionComponents/Container";
import MainHeading from "../Heading/MainHeading";
import Paragraph from "../Paragraph/Paragraph";

const Banner = () => {
  return (
    <section className="max-w-[1600px] mx-auto pt-2 pb-10">
      <Container>
        <div className="grid grid-cols-3 w-full items-center justify-center">
          <div className="col-span-1">
            <div className="relative h-[240px] aspect-[4/1.58] left-24">
              <Image
                src="/outside.jpeg"
                alt="alt"
                fill
                className="object-cover !z-20"
              />
            </div>
          </div>
          <div className="col-span-2 w-full ">
            <div className="relative w-full aspect-[4/2.6] ">
              <Image
                src="/outside.jpeg"
                alt="alt"
                fill
                className="object-cover"
              />
            </div>
          </div>
        </div>
        <div className="relative w-full max-w-4xl bg-white z-20 h-full -top-24  shadow-xl">
          <div className="flex flex-col h-full w-full lg:p-14 p-5 z-30 bg-white">
            <MainHeading title="Lorem ipsum, dolor sit amet consectetur adipisicing elit. Commodi" />
            {[1, 2].map((item, index) => (
              <Paragraph
                className="text-textdark !text-start my-4"
                text="Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam"
                key={index}
              />
            ))}
          </div>
        </div>
      </Container>
      <div className="absolute w-1/2 bg-bgclr -z-10 h-full top-0 left-0"></div>
    </section>
  );
};

export default Banner;
