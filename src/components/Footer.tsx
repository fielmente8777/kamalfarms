        ...
        <div className="grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 lg:gap-16 gap-8">
          {sections.map((section, index) => (
            <div className="flex flex-col gap-4" key={index}>
              <h3 className="text-lg font-semibold tracking-wide underline">{section.title}</h3>
          ...
          <div className="flex flex-col gap-4">
            <h3 className="text-lg font-semibold tracking-wide underline">Follow Us</h3>
            <ul className={`flex gap-4`}>
              <Facebook />
              <Insta />
            </ul>
            <span className=" flex gap-3"><CopyRight /> {" "}Kamal Farms, Karjat | 2020</span>
          </div>
        </div>
